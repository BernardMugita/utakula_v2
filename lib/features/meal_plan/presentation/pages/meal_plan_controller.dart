...
class MealPlanController extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    useEffect(() {
      if (userMealPlan != null && userMealPlan!.isNotEmpty) {
        final deepCopiedPlan = _deepCopyMealPlan(userMealPlan!);
        mealPlan.value = deepCopiedPlan;
        originalMealPlan.value = _deepCopyMealPlan(userMealPlan!);
      }
      return null;
    }, []);
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          elevation: 0,
          title: Text(
            userMealPlan == null || userMealPlan!.isEmpty
        ...
        body: SingleChildScrollView(
          ...
          child: Column(
            children: [
              InfoBanner(
                message: userMealPlan == null || userMealPlan!.isEmpty
  ...
  // Deep copy meal plan
  List<Map<String, dynamic>> _deepCopyMealPlan(List<dynamic> original) {
    return original
        .map(
          (item) => {
            'day': item['day'],
  ...
  bool _hasMealPlanChanged(
    ...
    for (int i = 0; i < original.length; i++) {
      ...
      for (var key in originalMeals.keys) {
        ...
        if (originalMeals[key].length != currentMeals[key].length) return true;
      }
    }
